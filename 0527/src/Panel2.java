...
public class Panel2 extends JPanel implements ActionListener, MouseListener {
    ...
    public Panel2(McMain mmain) {
        ...
        String[][] data = FileUtil.getFile("mainmenu.txt");

        // 이 이중 for문이 이중 배열의 전체값을 읽어오는 부분
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource()==btnPrev) {
            mmain.add(mmain.pnl1);
            // 메인을 내가(Panel2)가 들고있으니까 메인에게 명령을 할 수 있다.
            mmain.remove(this);
            ...
            if (isExist==false) {
                ...
                // tbl.setValueAt(바꿀 수량컬럼 값, 내가 찾은 행이 몇 번째, 두 번째 컬럼(수량컬럼));
            }
        }
    }
